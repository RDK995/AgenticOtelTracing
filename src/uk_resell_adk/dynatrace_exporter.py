...
try:
    ...
    ExportTraceServiceResponse = None


LOGGER = logging.getLogger(__name__)

_DESTINATION = "dynatrace"
    ...
    *,
    ...
    spans = [
        _SpanRow(
            ...
            resource_attributes=row[12] or {},
            ingestion_time=row[13],
        )
        for row in rows
    ]
    if not spans:
...
def _build_otlp_payload(
    ...
    spans_by_resource: dict[tuple[str, str | None, str], list[dict[str, Any]]] = defaultdict(list)

    for span in spans:
        key = (span.service_name, span.environment, json.dumps(span.resource_attributes, sort_keys=True))
        event_payload = [
            {
        ...
        spans_by_resource[key].append(
            {
                ...
                "attributes": _to_otel_attributes(span.attributes),
    ...
    for (service_name, environment, resource_json), grouped_spans in spans_by_resource.items():
        ...
        resource_spans.append(
            {
                ...
                "scopeSpans": [
                    {
                        "scope": {
                            "name": "uk_resell_adk.postgres_dynatrace_exporter",
                            "version": "0.1.0",
                        },
                        "spans": grouped_spans,
                    }
...
def _dynatrace_headers(api_token: str) -> dict[str, str]:
    headers = {
...
def _export_once(
    *,
    ...
    with psycopg.connect(db_dsn, autocommit=False) as conn:
        ...
        payload = _build_otlp_payload(spans=spans, events=events, links=links)
        if dry_run:
            LOGGER.info(
        ...
        try:
            if ExportTraceServiceRequest is None:
                raise RuntimeError(
                    "Dynatrace export requires OTLP protobuf definitions; "
            ...
            response = httpx.post(
                endpoint,
                headers=_dynatrace_headers(api_token=dynatrace_api_token),
                content=body,
                timeout=timeout_seconds,
            )
            response.raise_for_status()
            if ExportTraceServiceResponse is not None and response.content:
                try:
                    resp_proto = ExportTraceServiceResponse()
                    resp_proto.ParseFromString(response.content)
                    partial = getattr(resp_proto, "partial_success", None)
                    rejected = int(getattr(partial, "rejected_spans", 0) or 0) if partial is not None else 0
                    error_message = str(getattr(partial, "error_message", "") or "") if partial is not None else ""
                    if rejected > 0:
                        raise RuntimeError(
                            f"Dynatrace OTLP partial success: rejected_spans={rejected}, error={error_message}"
                        )
                except RuntimeError:
                    raise
                except Exception:
                    # If Dynatrace returns a non-OTLP body, ignore parse errors here.
                    pass
        ...
        conn.commit()
        LOGGER.info(
            "Exported %s spans to Dynatrace (%s). Watermark advanced to %s.",
            len(spans),
...
def _arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Export spans from Postgres OTEL schema to Dynatrace OTLP endpoint.")
    parser.add_argument(
        "--db-dsn",
        default=os.getenv(
            "OTEL_POSTGRES_DSN",
    ...
    parser.add_argument(
        ...
        default=os.getenv("DYNATRACE_OTLP_ENDPOINT", ""),
    )
    parser.add_argument("--dynatrace-api-token", default=os.getenv("DYNATRACE_API_TOKEN", ""))
    parser.add_argument("--batch-size", type=int, default=int(os.getenv("DYNATRACE_EXPORT_BATCH_SIZE", "200")))
    parser.add_argument("--timeout-seconds", type=float, default=float(os.getenv("DYNATRACE_EXPORT_TIMEOUT_SECONDS", "20")))
    parser.add_argument("--poll-interval-seconds", type=float, default=float(os.getenv("DYNATRACE_EXPORT_POLL_SECONDS", "5")))
    parser.add_argument("--continuous", action="store_true")
    parser.add_argument("--dry-run", action="store_true")
    parser.add_argument("--log-level", default=os.getenv("DYNATRACE_EXPORT_LOG_LEVEL", "INFO"))
    return parser
...
def main() -> None:
    ...
    if not args.dry_run:
        if not args.dynatrace_otlp_endpoint:
            raise SystemExit("DYNATRACE_OTLP_ENDPOINT is required for Postgres->Dynatrace export.")
        if not args.dynatrace_api_token:
            raise SystemExit("DYNATRACE_API_TOKEN is required for Postgres->Dynatrace export.")

    while True:
        exported = _export_once(
            db_dsn=args.db_dsn,
            dynatrace_otlp_endpoint=args.dynatrace_otlp_endpoint,
            dynatrace_api_token=args.dynatrace_api_token,
