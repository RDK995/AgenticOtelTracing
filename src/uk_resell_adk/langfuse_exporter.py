...
def _build_otlp_payload(
    ...
    def _enriched_attributes(span: _SpanRow) -> dict[str, Any]:
        ...
        if span.parent_span_id is None:
            ...
            if span.user_id:
                ...
                attrs.setdefault("langfuse.user.id", span.user_id)
            if span.session_id:
                attrs.setdefault("session.id", span.session_id)
                attrs.setdefault("langfuse.session.id", span.session_id)
            if span.trace_input:
    ...
    for span in spans:
        ...
        link_payload = [
            {
                "traceId": link.linked_trace_id,
                "spanId": link.linked_span_id,
...
def _export_once(
    *,
    ...
    with psycopg.connect(db_dsn, autocommit=False) as conn:
        ...
        conn.commit()
        LOGGER.info(
            "Exported %s spans to Langfuse (%s). Watermark advanced to %s.",
            len(spans),
...
def _arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Export spans from Postgres OTEL schema to Langfuse OTLP endpoint.")
    parser.add_argument(
        "--db-dsn",
        default=os.getenv(
            "OTEL_POSTGRES_DSN",
    ...
    parser.add_argument("--langfuse-host", default=os.getenv("LANGFUSE_BASE_URL", "https://cloud.langfuse.com"))
    ...
    parser.add_argument("--batch-size", type=int, default=int(os.getenv("LANGFUSE_EXPORT_BATCH_SIZE", "200")))
    parser.add_argument("--timeout-seconds", type=float, default=float(os.getenv("LANGFUSE_EXPORT_TIMEOUT_SECONDS", "20")))
    parser.add_argument("--poll-interval-seconds", type=float, default=float(os.getenv("LANGFUSE_EXPORT_POLL_SECONDS", "5")))
    parser.add_argument("--continuous", action="store_true")
    parser.add_argument("--dry-run", action="store_true")
    parser.add_argument("--log-level", default=os.getenv("LANGFUSE_EXPORT_LOG_LEVEL", "INFO"))
    return parser
...
def main() -> None:
    ...
    if not args.dry_run and (not args.langfuse_public_key or not args.langfuse_secret_key):
        raise SystemExit(
            "LANGFUSE_PUBLIC_KEY and LANGFUSE_SECRET_KEY are required for Postgres->Langfuse export."
        )
